...
const litesql::FieldType Contacts::Extprop_tags("extprop_tags",A_field_type_string,table__);
const litesql::FieldType Contacts::Extprop_away_since("extprop_away_since",A_field_type_integer,table__);
...
Contacts::Contacts(const litesql::Database& db, const litesql::Record& rec)
    ...
    defaults();
    size_t size = (rec.size() > 84) ? 84 : rec.size();
    switch(size) {
    case 84: extprop_away_since = convert<const std::string&, int>(rec[83]);
        extprop_away_since.setModified(false);
    case 83: extprop_tags = convert<const std::string&, std::string>(rec[82]);
        extprop_tags.setModified(false);
...
const Contacts& Contacts::operator=(const Contacts& obj) {
    if (this != &obj) {
        ...
        extprop_tags = obj.extprop_tags;
        extprop_away_since = obj.extprop_away_since;
    }
    litesql::Persistent::operator=(obj);
    return *this;
}
std::string Contacts::insert(litesql::Record& tables, litesql::Records& fieldRecs, litesql::Records& valueRecs) {
    ...
    liveid_cid.setModified(false);
    fields.push_back(extprop_tags.name());
    values.push_back(extprop_tags);
    extprop_tags.setModified(false);
    fields.push_back(extprop_away_since.name());
    values.push_back(extprop_away_since);
    extprop_away_since.setModified(false);
    fieldRecs.push_back(fields);
    valueRecs.push_back(values);
    return litesql::Persistent::insert(tables, fieldRecs, valueRecs, sequence__);
...
void Contacts::addUpdates(Updates& updates) {
    ...
    updateField(updates, table__, extprop_tags);
    updateField(updates, table__, extprop_away_since);
}
...
void Contacts::getFieldTypes(std::vector<litesql::FieldType>& ftypes) {
    ...
    ftypes.push_back(Extprop_tags);
    ftypes.push_back(Extprop_away_since);
}
...
std::auto_ptr<Contacts> Contacts::upcastCopy() const {
    ...
    np->extprop_tags = extprop_tags;
    np->extprop_away_since = extprop_away_since;
    np->inDatabase = inDatabase;
...
std::ostream & operator<<(std::ostream& os, Contacts o) {
    ...
    os << o.extprop_tags.name() << " = " << o.extprop_tags << std::endl;
    os << o.extprop_away_since.name() << " = " << o.extprop_away_since << std::endl;
    os << "-------------------------------------" << std::endl;
    return os;
}
